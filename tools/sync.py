...
def sync_from_test(dry_run=False, push_to_git=False):
    ...
    if push_to_git:
        ...
        if not run_command(["git", "add", "."], cwd=MAIN_DIR):
            return

        commit_message = (
            f"Auto-sync from test environment on "
            f"{datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
        )
        print(f"Committing with message: '{commit_message}'")
        if not run_command(["git", "commit", "-m", commit_message], cwd=MAIN_DIR):
            return

        print("Pushing to remote repository...")
        if not run_command(["git", "push"], cwd=MAIN_DIR):
            return
...
def main():
    ...
    subparsers = parser.add_subparsers(
        dest="direction", required=True, help="The direction of the sync."
    )

    parser_to_test = subparsers.add_parser(
        "to-test", help="Sync essential source FROM main TO the test directory."
    )
    parser_to_test.set_defaults(func=sync_to_test)

    parser_from_test = subparsers.add_parser(
        "from-test",
        help="Sync ALL changes FROM test back TO the main project directory.",
    )
    parser_from_test.add_argument(
        "-n",
        "--dry-run",
        action="store_true",
    ...
    parser_from_test.add_argument(
        "--push",
        action="store_true",
        help="Commit and push changes to Git after syncing.",
    )
    parser_from_test.set_defaults(
        func=lambda args: sync_from_test(args.dry_run, args.push)
    )
    ...
    if not MAIN_DIR.is_dir() or not TEST_DIR.is_dir():
        print(
            f"Error: Ensure both '{MAIN_DIR}' and '{TEST_DIR}' exist.",
            file=sys.stderr,
        )
        sys.exit(1)
