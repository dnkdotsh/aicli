...
from aicli import config # For raw_log_file path
...
class TestApiClient:
    ...
    def test_make_api_request_success(self, mock_requests_post, mock_openai_chat_response, fake_fs):
    ...
    def test_make_api_request_http_error(self, mock_requests_post, mocker, capsys, fake_fs):
        ...
        mock_response.json.side_effect = requests.exceptions.JSONDecodeError("Expecting value", "doc", 0) # Simulate non-JSON error body
    ...
    def test_make_api_request_json_decode_error(self, mock_requests_post, mocker, capsys, fake_fs):
        ...
        assert re.search(r"Failed to decode API response|Expecting value", str(excinfo.value))
        assert config.RAW_LOG_FILE.exists()

    def test_make_api_request_timeout(self, mock_requests_post, mocker, capsys, fake_fs):
    ...
    def test_redact_sensitive_info_auth_header(self):
        ...
        log_entry = {
            "request": {
                ...
                "headers": {"Authorization": "Bearer sk-12345ABCDEF", "Content-Type": "application/json"},
                "payload": {"prompt": "test"}
            }
        }
        redacted_entry = api_client._redact_sensitive_info(log_entry)
        assert redacted_entry["request"]["headers"]["Authorization"] == "Bearer [REDACTED]"
        assert log_entry["request"]["headers"]["Authorization"] == "Bearer sk-12345ABCDEF" # Original should be unchanged
    ...
    def test_redact_sensitive_info_query_param(self):
        ...
        redacted_entry = api_client._redact_sensitive_info(log_entry)
        assert redacted_entry["request"]["url"] == "https://generativelanguage.googleapis.com/v1beta/models/gemini:generateContent?key=[REDACTED]"
        assert log_entry["request"]["url"] == "https://generativelanguage.googleapis.com/v1beta/models/gemini:generateContent?key=AIzaSyABCD" # Original unchanged

    def test_perform_chat_request_streaming_error(self, mock_requests_post, mocker, capsys):
        ...
        mock_response = mocker.MagicMock(spec=requests.Response)
        mock_response.status_code = 200
        mock_response.iter_lines.side_effect = requests.exceptions.RequestException("Stream connection reset")
        mock_requests_post.return_value = mock_response

        response_text, tokens = api_client.perform_chat_request(
            engine=mock_engine, model="gpt-4o-mini", messages_or_contents=[],
            system_prompt=None, max_tokens=100, stream=True
        )
