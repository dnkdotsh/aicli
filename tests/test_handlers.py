...
def test_handle_chat_single_shot(mocker, capsys):
    ...
    mock_perform_chat = mocker.patch(
        ...
        return_value=(
            "Test response.",
            {"prompt": 1, "completion": 2, "total": 3, "reasoning": 0},
        ),
    )
...
def test_handle_chat_interactive_mode_setup(mocker, mock_openai_engine):
    ...
    """
    mock_perform_interactive_chat = mocker.patch(
        "aicli.handlers.perform_interactive_chat"
    )
...
def test_handle_chat_single_shot_with_attachments_and_system_prompt(mocker):
    ...
    """
    mock_perform_chat = mocker.patch(
        "aicli.api_client.perform_chat_request", return_value=("Response", {})
    )
    mock_engine = OpenAIEngine("fake_key")
...
def test_handle_image_generation(mocker, fake_fs):
    ...
    handlers.handle_image_generation(
        api_key="fake_key", engine=mock_engine, prompt="A test image"
    )
...
class TestMultiChatHandler:
    ...
    def setup_multichat_mocks(self, mocker):
        ...
        mocker.patch(
            "aicli.handlers.get_engine",
            side_effect=lambda name, key: self.mock_openai_engine
            if name == "openai"
            else self.mock_gemini_engine,
        )
        self.mock_perform_multichat = mocker.patch(
            "aicli.handlers.perform_multichat_session"
        )
    ...
    def test_handle_multichat_missing_api_key_exits(self, mocker, caplog):
        ...
        """
        mocker.patch(
            "aicli.api_client.check_api_keys",
            side_effect=api_client.MissingApiKeyError("Missing Key"),
        )
...
class TestSelectModel:
    ...
    def test_select_model_invalid_selection_falls_back_to_default(
        self, mocker, capsys, mock_prompt_toolkit
    ):
    ...
    def test_select_model_no_available_models_falls_back_to_default(
        self, mocker, capsys, mock_prompt_toolkit
    ):
