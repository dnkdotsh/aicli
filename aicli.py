...
def main():
    ...
    if len(sys.argv) == 1 and sys.stdin.isatty():
        ...
        try:
            ...
            handlers.handle_chat(engine_instance, model, None, None, [], None, settings.get('default_max_tokens'), settings.get('stream'), False, False)
    ...
    # Mutually exclusive group for chat/image/both modes
    ...
    context_group.add_argument('--memory', action='store_true', help='Load the persistent memory file into context.')
    ...
    if not args.chat and not args.image and not args.both:
        args.chat = True # Default to chat mode if no other mode is specified
    ...
    try:
        if args.both:
            ...
            handlers.handle_chat(engine_instance, model, system_prompt, prompt, image_data, args.session_name, args.max_tokens, args.stream, args.memory, args.debug)
        elif args.image:
            api_key = api_client.check_api_keys(args.engine)
            engine_instance = get_engine(args.engine, api_key)
