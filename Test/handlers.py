...
def _perform_chat_request(api_key: str, engine: str, model: str, messages_or_contents: list, system_prompt: str | None, max_tokens: int, stream: bool, session_raw_logs: list | None = None) -> tuple[str, dict]:
    ...
    if engine == 'openai':
        ...
        # --- MAJOR FIX: Correctly construct the streaming URL ---
        # Rationale: The previous logic created a malformed URL with two '?' characters,
        # causing a 400 Bad Request error. This now correctly uses '&' for the second parameter.
        if stream:
...
def _perform_interactive_chat(api_key: str, engine: str, model: str, system_prompt: str, initial_image_data: list, session_name: str, max_tokens: int, stream: bool):
    ...
    try:
        while True:
            ...
            # --- TINY FIX: Conditionally print "Assistant:" prefix ---
            # Rationale: For streaming, we print it before the response comes. For non-streaming,
            # we now print it *with* the response to avoid the awkward empty line while waiting.
...
def _update_persistent_memory(api_key: str, engine: str, model: str, log_file: str):
    ...
    try:
        ...
        summary_text, _ = _perform_chat_request(api_key, engine, model, messages_or_contents, None, 1024, stream=False)
